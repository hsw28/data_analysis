...
cp -rf $RPM_BUILD_DIR/fkSession/* $RPM_BUILD_ROOT/opt/matlabR14/toolbox/%{name}
...
sed -i -e "s/\(<help_location>\).*\(<\/help_location>\)/\1\$docroot\/toolbox\/fkSession\2/" $RPM_BUILD_ROOT/opt/matlabR14/toolbox/%{name}/info.xml

#change start menu item to use doc instead of help
sed -i -e "s/help\( fkSession\)/doc\1/" $RPM_BUILD_ROOT/opt/matlabR14/toolbox/%{name}/info.xml
...
if [ "$1" = "0" ] ; then  # last uninstall
  sed -i -e "/fkSession/d" $RPM_INSTALL_PREFIX/toolbox/local/pathdef.m
fi
