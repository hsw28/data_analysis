...
cp -rf $RPM_BUILD_DIR/fkUtilities/* $RPM_BUILD_ROOT/opt/matlabR14/toolbox/%{name}
...
sed -i -e "s/\(<help_location>\).*\(<\/help_location>\)/\1\$docroot\/toolbox\/fkUtilities\2/" $RPM_BUILD_ROOT/opt/matlabR14/toolbox/%{name}/info.xml

#change start menu item to use doc instead of help
sed -i -e "s/help\( fkUtilities\)/doc\1/" $RPM_BUILD_ROOT/opt/matlabR14/toolbox/%{name}/info.xml
...
#  cp /opt/matlabR14/toolbox/local/pathdef.m /opt/matlabR14/toolbox/local/pathdef.m.backup
  sed -i -e "s/[ ]\{4,\}[.]\{3\}/matlabroot,'\/toolbox\/fkUtilities:',...\n     .../g" $RPM_INSTALL_PREFIX/toolbox/local/pathdef.m
fi
...
if [ "$1" = "0" ] ; then  # last uninstall
  sed -i -e "/fkUtilities/d" $RPM_INSTALL_PREFIX/toolbox/local/pathdef.m
fi
