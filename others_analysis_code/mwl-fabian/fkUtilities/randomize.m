...
if numel(groupdim)>=1
  switch options.adjust
   case 'mean'
    adjust_factor = mean( M(I),3 );
   case 'max'
    adjust_factor = max( M(I),[],3 );
  end
end

[row_i, col_i, plane_i] = ndgrid( 1:dx, 1:dy, 1:dz ); %#ok

switch options.method
 case 'cycle'
  row_i = randcycle( row_i(:,:,1) );
 otherwise
  [row_i,row_i] = sort(rand([dx dy]),1); %#ok
end
...
if numel(groupdim)>=1
  switch options.adjust
   case 'mean'
    adjust_factor = adjust_factor./mean( M(I),3 );
    M(I) = bsxfun(@times, M(I), adjust_factor );
   case 'max'
    adjust_factor = adjust_factor./max( M(I),[],3 );
    M(I) = bsxfun(@times, M(I), adjust_factor );    
  end
end
