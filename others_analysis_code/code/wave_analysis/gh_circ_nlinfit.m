function [beta,r,J,Sigma,mse] = gh_circ_nlinfit(X,y,model,beta,options)
...
%   $Revision: 2.22.2.13 $  $Date: 2007/09/18 02:34:36 $
...
options = statset(options,'MaxIter',1000);
...
%r = y(:) - yfit(:); % GREG MODIFICATION
r = gh_circular_subtract(y(:),yfit(:)); % GREG MODIFICATION
r(nans) = [];
n = numel(r);
p = numel(beta);
sse = r'*r;
...
yfit = model(beta,x);
%fullr = y(:) - yfit(:); % GREG MODIFICATION
fullr = gh_circular_subtract(y(:),yfit(:)); % GREG MODIFICATION
ok = ~isnan(fullr);
r = fullr(ok);
