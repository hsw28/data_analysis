...
% can't test whether 'isa' 'filt' object, so:
  ...
  %  else
    ...
    c.data(1:end-Gd,:) = c.data(Gd+1:end,:);
    c.data(end-Gd+1:end,:) = 0;
...
% we could also do this using the design paramters of the filter, now
% that we have those in the filt structure.
