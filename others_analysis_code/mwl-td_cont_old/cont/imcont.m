...
%  -'timestamp' = timestamps returned from mwleegfile/load
...
%  'timeunits': 'timestamps' or 'seconds'; default depends on inputs
%
% output struct:
%
% $$$    -contdata struct:
...
% hassle, esp chanvals and chanlabels
  ...
  if ~isempty(eegfh);
    ...
    % note this is usu off by a small amount. (rate according to the A->D card's
    ...
    if ~isempty(a.data) && ~isempty(a.timestamp);
      ...
      if ~isempty(a.timewin),
        ...
        buff_t = double(buff_ts) ./ ts_per_sec;
        ...
        if max(diff(buff_t(loadbuffs(1):loadbuffs(2)))) > ...
              (1.5 * t_per_buff),
          error(['requested eeg range contains gaps (recording stopped?' ...
                 ')']);
        end
...
% $$$       c.datarange = a.eeg.info.ADCrange .* max(adunits_to_mv_f);
  ...
  switch timeunits
   case 'seconds'
    % no conversion
    if timestamp(end) > 50 * ts_per_sec;
      warning(['timestamps reported to be in seconds, may be in timestamp ' ...
               'units']);
    end
   case 'timestamps'
    timestamp = double(timestamp) ./ ts_per_sec;
   otherwise
  ...
  c.max_tserr = max(timestamp - ts_syn(:));
  ...
  if c.max_tserr > 0.5*(1/c.samplerate),
    warning(['max error in computed timestamp will be greater than 50% of ' ...
