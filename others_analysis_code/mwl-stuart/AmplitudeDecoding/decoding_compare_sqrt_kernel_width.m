...
tic;
for i=1:numel(kw)
    disp(['Decoding: ', num2str(i), ' of: ' , num2str(numel(amps)), ' estimates']);
    [est{i} tbins pbins] = decode_amplitudes(amps{1}, pos', t_range, d_range,...
        'amp_kw', repmat(kw(i),1,4), 'wb',1,'wb_color', 'y');
    method{i} = ['', num2str(kw(i)), ' uV'];
end
t = toc;

disp(['Decoded: ', num2str(diff(d_range)), ' seconds of data ', num2str(numel(est_sqr)), ' times in ', num2str(t), ' seconds']);

%%
plot_amp_decoding_estimate_errors(est_sqr,exp.(ep).pos, 'decode_range', d_range, 'legend', method_sq, 'n_spike', ns_sqr);