function f = firingphaseDEPRECATED(firingtimes, lfp, timevector, vel, above, bins)
...
while i<= size(firingtimes,1)
  z = find(abs(firingtimes(i)-goodtimes)<.0005);

  if length(z)>=1

     x = find( abs(peaktimes-firingtimes(i)) < .16);

	    %makes sure we only have one point. point is the time of the closest peak
		if length(x) == 0 | length(x)>3 %this means no peaks satisfy or too many peaks to be theta
			i = i+1;
      %if some peaks satisfy
		else
			if length(x) == 1 %if one peak only satisfies, you're golden
				point = peaktimes(x);
			elseif length(x) == 2 | length(x)==3 % if more than one peak satisfies, take the closer point
				r = 1;
				z = 100;
				closer = [];
				while r<=size(x,1)
