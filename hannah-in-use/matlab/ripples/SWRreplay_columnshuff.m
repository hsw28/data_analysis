function confidence9599 = SWRreplay_columnshuff(SWRstartend, pos, clusters, tdecode, num_of_sims)
%to control for the chance linear alignment of position estimates, we circularly shifted the estimate at each time point by a random distance (‘‘column-cycle shuffle’’).
...
newmat = matrices_currentrip([traj1],:);
for s = 1:size(newmat,2)
  newmat(:,s) = circshift(newmat(:,s), randi(size(newmat,1)+1)-1);
end

  [slope_new, intercept_new, score_new, projection_new]  = est_line_detect([(1:1:size(matrices_currentrip,2))/timestep], [1:size(matrices_currentrip([traj1],:))], newmat);
  ...
  newmat = matrices_currentrip([traj2],:);
  for s = 1:size(newmat,2)
    newmat(:,s) = circshift(newmat(:,s), randi(size(newmat,1)+1)-1);
  end
  [slope_new, intercept_new, score_new, projection_new]  = est_line_detect([(1:1:size(matrices_currentrip,2))/timestep], [1:size(matrices_currentrip([traj2],:))], newmat);
  if score_new > score_old
    score_old = score_new;
    score = score_new;
    slope = slope_new;
  ...
  newmat = matrices_currentrip([traj3],:);
  for s = 1:size(newmat,2)
    newmat(:,s) = circshift(newmat(:,s), randi(size(newmat,1)+1)-1);
  end
  [slope_new, intercept_new, score_new, projection_new]  = est_line_detect([(1:1:size(matrices_currentrip,2))/timestep], [1:size(matrices_currentrip([traj3],:))], newmat);
  if score_new > score_old
    score_old = score_new;
    score = score_new;
    slope = slope_new;
  ...
  newmat = matrices_currentrip([traj4],:);
  for s = 1:size(newmat,2)
    newmat(:,s) = circshift(newmat(:,s), randi(size(newmat,1)+1)-1);
  end
  [slope_new, intercept_new, score_new, projection_new]  = est_line_detect([(1:1:size(matrices_currentrip,2))/timestep], [1:size(matrices_currentrip([traj4],:))], newmat);
  if score_new > score_old
    score_old = score_new;
    score = score_new;
    slope = slope_new;
  ...
  newmat = matrices_currentrip([traj5],:);
  for s = 1:size(newmat,2)
    newmat(:,s) = circshift(newmat(:,s), randi(size(newmat,1)+1)-1);
  end
  [slope_new, intercept_new, score_new, projection_new]  = est_line_detect([(1:1:size(matrices_currentrip,2))/timestep], [1:size(matrices_currentrip([traj5],:))], newmat);
  if score_new > score_old
    score_old = score_new;
    score = score_new;
    slope = slope_new;
  ...
  newmat = matrices_currentrip([traj6],:);
  for s = 1:size(newmat,2)
    newmat(:,s) = circshift(newmat(:,s), randi(size(newmat,1)+1)-1);
  end
  [slope_new, intercept_new, score_new, projection_new]  = est_line_detect([(1:1:size(matrices_currentrip,2))/timestep], [1:size(matrices_currentrip([traj6],:))], newmat);
  if score_new > score_old;
    score_old = score_new;
    score = score_new;
    slope = slope_new;
