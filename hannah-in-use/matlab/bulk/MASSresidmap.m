function f = MASSresidmap(spikestructure, posstructure, dim, vel_or_acc)

%finds place fields with firing via vel/acc removed
...
oldpos = 0;
  for k = 1:spikenum
      ...
      veldata = posstructure.(velformateddate);
      accdata = abs(posstructure.(accformateddate));
    ...
    if newdate ~= previousdate
      ...
      velpowerchart = powermap(veldata, currentpos, dim, 0, 50);
      accpowerchart = powermap(accdata, currentpos, dim, 0, 50);
    end
      maxtime = max(currentpos(:,1));
      currentpos = [currentpos; [maxtime+.03, 0, 0]; [maxtime+.06, 1500, 1500]];
      spikechart = normalizePosData(spikestructure.(name), currentpos, dim);
      reshapesize = size(powerchartacc);
