function c = csi(clusters, spike_amp, interval)
...
spike_times = struct2cell(clusters);


%all = zeros(10,1);
%for n=1:numclust
%  name = char(clustname(n));
%  all = padcat(all', clusters.(name)');
%  size(clusters.(name))
%end
%spike_times = all;
...
for i=1:n_spike_vectors
    ...
    for j=1:n_spike_vectors
        ...
        elseif i==j
            ...
            c(end+1) = calccsi(dt, da, max_int, min_int) ./ numel(spike_times{j});
        end
    end
